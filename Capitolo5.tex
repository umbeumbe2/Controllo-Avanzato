...
\end{pmatrix}
\]
Andiamoci quindi a calcolare la funzione di trasferimento ad anello chiuso
tra l'ingresso \(W(s)\) e l'uscita \(Z(s)\), ricordando
che vale \ref{funzione_trasferimento_H_infinito}:
\[
    T_{zw}(s) = \bar{G}_{11}(s) + \bar{G}_{12}(s) 
    (1 - \bar{G}_{22}(s))^{-1} \bar{G}_{21}(s)
    = 
    \begin{pmatrix}
        W_{S}(s) \\
        0 \\
    ...
    (I + G(s) K(s))^{-1}
\]
Ricordando che \(S(s) = \left(1 + G(s)K(s)\right)^{-1}\),
si ha:
\[T_{zw}(s) = 
    \begin{pmatrix}
        W_{S}(s) \\
    ...
    =
    \begin{pmatrix}
        W_{S}(s) -W_{S}(s) G(s) K(s) S(s) \\
        W_{K}(s) K(s) S(s) \\
        W_{T}(s) G(s) K(s) S(s)
    \end{pmatrix}
\]
Ricorndano che \(T(s) = G(s) K(s) S(s)\), si ha:
\[T_{zw}(s) = 
    \begin{pmatrix}
        W_{S}(s) -W_{S}(s) G(s) K(s)S(s) \\
        W_{K}(s) K(s) S(s) \\
    ...
    \end{pmatrix}
\]
Mettendo in evidenza \(W_{S}(s)\) nella prima riga si ha:
\[T_{zw}(s) = 
    \begin{pmatrix}
        W_{S}(s) (I - G(s)K(s) S(s)) \\
        W_{K}(s) K(s) S(s) \\
        W_{T}(s) T(s)
    \end{pmatrix}
\]
Ricordando che \(T(s) = G(s) K(s) S(s)\), 
e che \(T(s) + S(s) = I\), si ha:
\[T_{zw}(s) = 
    \begin{pmatrix}
        W_{S}(s) S(s) \\
        W_{K}(s) K(s) S(s) \\
    ...
    \end{pmatrix}
\]
Quindi abbiamo dimostrato che il problema di \(H_{\infty}\)
associato al sistema fittizio è uguale 
al problema di mixed sensitivity design. Quindi possiamo andare a risolvere
il problema di mixed sensitivity design risolvendo il problema
\(H_{\infty}\) sul sistema fittizio.\\
\begin{figure}[tbp]
\centering
\begin{tikzpicture}[auto, node distance=2cm, >=latex']
...
Ora andiamo a riassumere la sequenza di passi per andare a formulare
il problema di \(H_{\infty}\) standard associato al 
problema di mixed sensitivity design:
\begin{itemize}
...
La soluzione del problema \(H_{\infty}\) ci da in uscita:
\begin{itemize}
    \item Il controllore \(K(s)\)
...
\right\|_\infty < \gamma
 \]
\end{itemize}
Ora per noi il problema di mixed sensitivity design 
è risolto se e solo se il valore di \(\gamma >0\) in 
uscita dal problema \(H_{\infty}\) (associato al
 problema di mixed sensitivity) è minore di 1:
\[\gamma < 1 
\Longrightarrow 
...
\section{Progettazione delle funzioni di peso}
Andiamo ora a vedere come progettare le funzioni di peso
\(W_{S}(s)\), \(W_{T}(s)\) e \(W_{K}(s)\) a partire dalle specifiche
di progetto.
