...
a valore maggiore e completamente contenuta in \(B_{\varepsilon}(\bar{x})\), come 
...
\begin{itemize}
  \item Nel caso in cui si ha \(\dot{V}(x) = 0\) la traiettoria \(x(t)\) al massimo può 
  rimanre sulla curva di livello a valore \(\bar{V}\) (dunque
  rimane confinata in \(B_{\varepsilon}(\bar{x})\)) 
  \item  Se  \(\dot{V}(x) < 0\) allora la traiettoria \(x(t)\) si muove verso l'interno
